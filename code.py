...
class Codeline:
    ...
    def desloc_codeline(self):
        if self.ypos > SCREEN_HEIGHT or len(self.ypos_list) > 20:
            ...
            screen.fill(YELLOW, (self.xpos, self.ypos_list[0], text.get_width() + FONT_SIZE, text.get_height()* self.fill))
            ...
            text = font.render(message, True, WHITE)
            screen.fill(YELLOW, (self.xpos, self.ypos_list[-1], text.get_width()+FONT_SIZE, text.get_height()))
...
while True:
    ...
    if current_time - color_time > 0: #10000 = 10 seconds
